...
export async function deps (options) {
  ...
  if (!config.skipMissing) {
    ...
    if (missing.size > 0) {
      const entries = Array.from(missing.entries()).map(([name, version]) => `  ${name}: ${version}`).join('\n')
      console.log(dedent`
        # Missing dependencies: ${missing.size}
        ${entries}
  ...
  if (!config.skipUnused && !options.update) {
  ...
  const outdatedList = Object.entries(outdated).map(([name, info]) => {
    return `  ${name}: ${info.current} -> ${info.latest}`
  }).join('\n')
...
async function installUpdates ({ cwd, outdated, declared }) {
  ...
  for (const name of Object.keys(outdated)) {
    ...
    const section = meta?.section || 'dependencies'
    bySection[section].push(`${name}@latest`)
  }

  for (const [section, packages] of Object.entries(bySection)) {
    ...
    console.log(`Installing ${packages.length} ${section}: ${packages.join(', ')}`)

    try {
      await runNpmCommand(args, { cwd })
    } catch (error) {
      console.warn(`Failed to update ${section}. ${error instanceof Error ? error.message : error}`)
    }
  }
}
